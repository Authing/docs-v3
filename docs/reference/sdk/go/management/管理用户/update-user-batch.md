...
| metadata | object | 否 | 数据对象数据，传入的对象中的 key 必须先在用户数据对象相关自定义字段   |  `{"school":"北京大学","age":22}` |
