...
`ManagementClient.update_pipeline_function`
...
| funcName | string | 否 | - | 函数名称  | `每周日凌晨 3-6 点系统维护禁止注册/登录` |
| funcDescription | string | 否 | - | 函数描述  | `每周日凌晨 3-6 点系统维护禁止注册/登录。` |
| sourceCode | string | 否 | - | 函数源代码。如果修改之后，函数会重新上传。  | `async function pipe(user, context, callback) {
  const date = new Date();
  const d = date.getDay();
